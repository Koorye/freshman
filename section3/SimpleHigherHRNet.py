...
from model.higher_hr_net import HigherHRNet
    ...
    def _predict_batch(self, image):
        with torch.no_grad():
            ...
            for idx, scale in enumerate(scales):
                ...
                #
                # 1. 如果images长度小于等于一次batch，直接通过预测
                #    否则，按照每次batch分割逐次通过预测
                #    预测的结果是一个二元组 [(b,nof_joints*2,h/4,w/4), (b,nof_joints,h/2,w/2)]
                #
                # 2. 按照最大分辨率的输出(即二元组的最后一个元素的尺寸(h/2,w/2))
                #    对二元组中每个元素(除最后一个之外)使用插值法进行上采样
                #    使其尺寸相同 -> (b,nof_joints*2,h/2,w/2)
                #    取每个元素的[:,:nof_joints]部分(前半部分)求均值作为heatmap
                #    (虽然heatmaps是列表，但是其只会有一个元素？)
                #    并取每个元素的[:,nof_joints:]部分(后半部分，如果后半部分存在)加入tags列表中
                #
                # 3. 如果尺寸重置过，还需要对每个heatmap和tag运用插值法进行尺寸的还原
                #    (事实上只有一个scale为1，尺寸不会被重置)
                # 
                # 最终返回的结果：
                # outputs: [(b,nof_joints*2,h/4,w/4), (b,nof_joints,h/2,w/2)]
                # heatmaps: [(b,nof_joints,h/2,w/2)]
                # tags: [(b,nof_joints,h/2,w/2)]
                ...
                # 1. 如果scale为1，则将每个tag拓展成四维 
                # (nof_joints,h/4,w/4) -> (1,nof_joints,h/4,w/4)后加入tags_list
                # 2. 取heatmaps[0]作为heatmaps的均值
                # 3. 如果final_heatmaps为空，将heatmaps[0]作为final_heatmaps
                #    否则...
                # (事实上，由于scales只有一种，该循环只会执行一次，
                # 第一次final_heatmaps为空，因此final_heatmaps即为heatmaps[0])
                # 
                # 最终返回的结果：
                # heatmaps_list: (b,nof_joints,h/2,w/2)
                # tags_list: [(b,nof_joints,h/2,w/2)]
                heatmaps_list, tags_list = aggregate_results(
                    scale, heatmaps_list, tags_list, heatmaps, tags, with_flip=False, project2image=True
                )

            # 根据scales归一化heatmap -> (b,nof_joints,h/2,w/2)
            heatmaps = heatmaps_list.float() / len(scales)

            # 拼接tags_list -> (b,nof_joints,h/2,w/2)
            ...
            # 解析heatmap得到每个人物的每个关键点的坐标和置信度
            # 1. 使用最大值池化对heatmap进行非极大值抑制(取出每个卷积核区域的极大值)
            # 2. 将heatmap的h和w变换到同一维度 (b,n_joints,h/2,w/2) -> (b,n_joints,1/4*h*w)
            # 3. 取出heatmap第二维(最后一维)最大的前max_nof_people(默认30)数量的值val_k和下标ind (b,nof_joints,max_nof_people)
            #    (对于每张图片的每种类型的关键点，都取出max_nof_people数量的最大值和下标)
            # 4. tags (b,nof_joints,h/2,w/2) -> (b,nof_joints,1/4*h*w,1)
            # 5. 取出tags中对应heatmap中最大值的下标位置对应的值tag_k进行拼接 (b,nof_joints,max_nof_people,1)
            #    (此处只有1个tag，故最后一维为1) 之后对tag_k中的每个元素取整
            # 6. 下标ind每个元素对宽度w求余得到x坐标，对宽度w整除得到y坐标，叠加得到ind_k (b,nof_joints,max_nof_people,2)
            #    (因为之前heatmap从(h,w)转换为h*w，故经过求余和整除就可以还原，
            #    例如heatmap尺寸为(h,w)20*30，变换为600，原先的(5,10)就变为5*30+10->160
            #    故160%30->10, 160//30->5，就得到原先的x,y坐标)
            # 经过上述步骤得到
            # loc_k (ind_k): (b,nof_joints,max_nof_people,2) -> [[[[x,y]]]] 最大的前max_nof_people数量个元素的坐标
            # tag_k: (b,nof_joints,max_nof_people,1) 最大的前max_nof_people数量个元素对应的tag
            # val_k: (b,nof_joints,max_nof_people) 最大的前max_nof_people数量个元素的值
            #
            # 7. 根据nof_joints的数量返回对应的joints_dict(此处为COCO)
            #    字典中包含了各关节点的信息，连接方式和顺序
            # 8. 对于每个batch的loc_k (nof_joints,max_nof_people,2),
            #                  tag_k (nof_joints,max_nof_people,1),
            #                  val_k (nof_joints,max_nof_people)
            #    拼接loc_k, val_k[...,None], tag_k为joint_k (nof_joints,max_nof_people,4)
            # 9. 进一步过滤出ret (n_people,nof_joints,4) 其中n_people表示图片中实际检测到的人数
            # 10. 对每个关键点的坐标(当前为小数)进行进一步微调，以尽可能贴近对应的像素点 (整数)
            grouped, scores = self.output_parser.parse(
                heatmaps, tags, adjust=True, refine=True  # ToDo parametrize these two parameters
            )

            # get final predictions
            # 进行最终的仿射变换操作以获得最终关键点的位置
...
if __name__ == '__main__':
    ...
    points = hhrnet.predict(img).reshape(-1,3)
    for point in points:
        img = cv2.circle(img, (int(point[1]), int(point[0])), 5, (0,0,255), -1)
